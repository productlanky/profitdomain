  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        // 2. Financials
        const [depositsRes, investmentsRes, referralsRes, stockLogsRes] =
          await Promise.all([
        ...
        stockLogsRes.documents.forEach((log: any) => {
          ...
          const price = type?.includes("tesla")
            ? teslaPrice
            : type?.includes("spacex")
              ? spxPrice
              : type?.includes("neuralink")
                ? nrlkPrice
                : type?.includes("boring")
                  ? nrlkPrice
                  : 0;
        ...
        setTierName(tierMap[userTierLevel] || "Silver");
      ...
      <div className="flex flex-col gap-4">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          {userProfile?.kycStatus !== "approved" && (
            <Link
              href="/profile"
              className="flex items-center gap-2 bg-amber-500/10 text-amber-600 dark:text-amber-400 px-4 py-2 rounded-full text-sm font-medium border border-amber-500/20 hover:bg-amber-500/20 transition-colors"
            >
        ...
        {accountStatus === "reviewing_upgrade" && (
          <div className="flex items-center gap-3 p-4 rounded-2xl bg-blue-50 border border-blue-200 text-blue-800 shadow-sm dark:bg-blue-900/20 dark:border-blue-800/50 dark:text-blue-200">
        ...
        {accountStatus === "suspended" && (
          <div className="flex items-center gap-3 p-4 rounded-2xl bg-red-50 border border-red-200 text-red-800 shadow-sm dark:bg-red-900/20 dark:border-red-800/50 dark:text-red-200">
            ...
            <p className="text-sm font-medium">
              ...
              suspended. Please contact our support team for assistance.
            </p>
          </div>
        )}
      </div>
          ...
          <div className="relative z-10 flex flex-col h-full justify-between gap-8">
            <div className="flex justify-between items-start">
              ...
              <div className="flex flex-col items-end gap-2">
                ...
                {/* Small indicator on the card for current status */}
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-3xl p-6 shadow-sm flex flex-col justify-between">
          <div className="flex justify-between items-start mb-4">
            ...
            <span className="text-xs font-bold bg-gray-100 dark:bg-white/10 px-2 py-1 rounded-md text-gray-500">
              Monthly
            </span>
          </div>
          <div>
        ...
        <div className="bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-3xl p-6 shadow-sm flex flex-col justify-between">
          ...
          <Link
            href="/investments"
            className="mt-4 text-xs font-semibold text-brand-600 dark:text-brand-400 flex items-center hover:underline"
          >
          ...
          <div className="relative z-10">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">
                View requirements to unlock lower fees and higher limits.
              </p>
            </div>
            <Button
              variant="outline"
              size="sm"
              className="mt-4 w-full rounded-xl h-9 text-xs"
              asChild
            >
              <Link href="/rank">View Upgrade Options</Link>
            </Button>
          </div>
          {/* Decor */}

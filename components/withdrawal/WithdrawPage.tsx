...
export default function WithdrawPage() {
  ...
  const isRestricted =
    accountStatus === "dormant" || accountStatus === "suspended";

  // --- SUBMIT HANDLER ---
  const onSubmit = async (data: WithdrawFormFields) => {
    if (isRestricted)
      return toast.error("Account restricted. Withdrawals are disabled.");
    if (!profile) return;
    ...
    if (!profile.withdrawal_password)
      return toast.error("Set withdrawal password in profile");
    if (data.password !== profile.withdrawal_password)
      return toast.error("Incorrect password");

    const amt = parseFloat(data.amount);
    if (amt > availableBalance) return toast.error("Insufficient balance");
    if (profile.withdrawalLimit && amt > profile.withdrawalLimit)
      return toast.error(`Limit exceeded ($${profile.withdrawalLimit})`);

    if (
      data.method === "BTC" &&
      data.address &&
      !validateBitcoin(data.address)
    ) {
        ...
        <div className="grid lg:grid-cols-[1.4fr_1fr] gap-8">
          ...
          <div className="flex flex-col gap-6">
            ...
            {accountStatus === "dormant" && (
              <div className="flex items-center gap-3 p-4 rounded-[1.5rem] bg-red-50 border border-red-200 text-red-800 shadow-sm dark:bg-red-900/20 dark:border-red-800/50 dark:text-red-200">
                <ShieldAlert className="h-5 w-5 flex-shrink-0" />
                <p className="text-sm font-medium">
                  <strong>Account Dormant:</strong> Withdrawals are currently
                  restricted. Please contact support to upgrade and restore full
                  access.
                </p>
              </div>
            )}
            ...
            {accountStatus === "suspended" && (
              <div className="flex items-center gap-3 p-4 rounded-[1.5rem] bg-red-50 border border-red-200 text-red-800 shadow-sm dark:bg-red-900/20 dark:border-red-800/50 dark:text-red-200">
                ...
                <p className="text-sm font-medium">
                  <strong>Account Suspended:</strong> Your account has been
                  suspended. Withdrawals are disabled. Please contact support.
                </p>
              </div>
            )}

            <div
              className={`bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-[2rem] p-8 shadow-sm relative overflow-hidden min-h-[500px] ${isRestricted ? "opacity-75" : ""}`}
            >
              {/* ALERT BANNER (KYC / PASSWORD) */}
              {profile &&
              ...
              <form
                ...
                className={`pt-6 ${profile && (profile.kycStatus !== "approved" || !profile.withdrawal_password) ? "mt-6" : ""}`}
              >
                  ...
                  {step === 1 && (
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div
                          className={`group relative mt-4 flex items-center justify-center border-b-2 border-gray-100 py-4 transition-all duration-300 dark:border-white/10 ${!isRestricted && "focus-within:border-brand-500"}`}
                        >
                          ...
                          <Input
                            ...
                            className={`
                              ...
                              ${isRestricted ? "cursor-not-allowed opacity-70" : ""}
                            `}
                          />
                        </div>
                          ...
                          {[25, 50, 100].map((pct) => (
                            <button
                              ...
                              onClick={() =>
                                setValue(
                                  "amount",
                                  (availableBalance * (pct / 100)).toFixed(2),
                                )
                              }
                              className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${
                                isRestricted
                                  ? "bg-gray-100 text-gray-400 dark:bg-white/5 dark:text-white/30 cursor-not-allowed"
                                  : "bg-gray-100 dark:bg-white/10 text-gray-600 dark:text-gray-300 hover:bg-brand-500 hover:text-white"
                              }`}
                            >
                              {pct}%
                            </button>
                      ...
                      {/* Method Grid */}
                      <div>
                        <label className="text-xs font-bold uppercase text-gray-500 dark:text-gray-400 tracking-wider mb-4 block">
                          Select Destination
                        </label>
                          ...
                          {METHODS.map((m) => {
                            ...
                            return (
                              <div
                                ...
                                onClick={() =>
                                  !isRestricted &&
                                  setValue("method", m.value as any)
                                }
                                className={`
                                  ...
                                  ${!isRestricted && !isSelected ? "hover:border-brand-300" : ""}
                                `}
                              >
                                <div className="flex items-center gap-3">
                                  <div
                                    className={`p-2 rounded-lg ${isSelected ? "bg-brand-500 text-white" : "bg-gray-100 dark:bg-white/10 text-gray-500"}`}
                                  >
                      ...
                      <Button
                        ...
                        className={`w-full h-12 rounded-xl text-base font-semibold transition-all ${
                          isRestricted
                            ? "bg-gray-300 text-gray-500 dark:bg-white/10 dark:text-white/40 cursor-not-allowed"
          ...
          <div className="bg-gray-900 rounded-[2rem] p-8 text-white relative overflow-hidden flex flex-col h-full shadow-2xl">
            ...
            <div className="relative z-10 flex-1">
              ...
              <div className="space-y-6">
                <div className="flex justify-between items-end">
                  ...
                  <span
                    className={`text-3xl font-mono font-bold tracking-tight ${isRestricted ? "text-white/40" : ""}`}
                  >
                    $
                    {watchAmount
                      ? parseFloat(watchAmount).toLocaleString(undefined, {
            ...
            <div className="relative z-10 mt-auto pt-8">
              <div className="bg-white/5 rounded-xl p-4 border border-white/10">
                ...
                {watchAmount &&
                  parseFloat(watchAmount) > availableBalance &&
                  !isRestricted && (

...
export default function BuySharesPage() {
  ...
  const fetchUserAccount = async () => {
    try {
      ...
      setBalance(profileDoc.totalDeposit || 0);
      setDocumentId(profileDoc.$id);

      // Fetch Account Status
      setAccountStatus(profileDoc.accountStatus ?? "active");
    } catch (err) {
  ...
  const loadPrice = async () => {
    ...
    try {
      const price =
        company === "tesla"
          ? parseFloat(await fetchTeslaPrice())
          : await fetchStockPrice(company);
      setSharePrice(price);
    } catch (err) {
  ...
  // Determine if trading is locked
  const isRestricted =
  ...
  const handleBuy = async () => {
    if (isRestricted)
      return toast.error("Account restricted. Cannot execute trade.");
    ...
    <div className="flex justify-center py-8 animate-in fade-in duration-500">
      <div className="w-full max-w-6xl space-y-6">
        ...
        {accountStatus === "dormant" && (
          <div className="flex items-center gap-3 p-4 rounded-2xl bg-red-50 border border-red-200 text-red-800 shadow-sm dark:bg-red-900/20 dark:border-red-800/50 dark:text-red-200">
            <ShieldAlert className="h-5 w-5 flex-shrink-0" />
            <p className="text-sm font-medium">
              <strong>Account Dormant:</strong> Purchasing shares is currently
              restricted. Please contact support to upgrade and restore trading
              privileges.
            </p>
          </div>
        )}
        ...
        {accountStatus === "suspended" && (
          <div className="flex items-center gap-3 p-4 rounded-2xl bg-red-50 border border-red-200 text-red-800 shadow-sm dark:bg-red-900/20 dark:border-red-800/50 dark:text-red-200">
            ...
            <p className="text-sm font-medium">
              ...
              suspended. Trading is disabled. Please contact support.
            </p>
          </div>
        )}
        ...
        {/* MAIN GRID */}
        <div
          className={`grid lg:grid-cols-[1.5fr_1fr] gap-8 ${isRestricted ? "opacity-70" : ""}`}
        >
          {/* LEFT: CONTROLS */}
              ...
              <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
                {COMPANIES.map((c) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`h-10 w-10 rounded-full flex items-center justify-center mb-2 ${c.bg} ${c.color}`}
                      >
            ...
            <div className="bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-[1.5rem] p-6 shadow-sm">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-2">
                  <div
                    className={`h-8 w-8 rounded-lg flex items-center justify-center ${currentCompany.bg} ${currentCompany.color}`}
                  >
                ...
                <div className="flex bg-gray-100 dark:bg-white/10 p-1 rounded-xl">
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${isRestricted ? "cursor-not-allowed" : ""} ${mode === "shares" ? "bg-white dark:bg-gray-800 shadow-sm text-gray-900 dark:text-white" : "text-gray-500 hover:text-gray-700 dark:hover:text-gray-300"}`}
                  >
                    Shares
                  </button>
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${isRestricted ? "cursor-not-allowed" : ""} ${mode === "amount" ? "bg-white dark:bg-gray-800 shadow-sm text-gray-900 dark:text-white" : "text-gray-500 hover:text-gray-700 dark:hover:text-gray-300"}`}
                  >
                    USD
                  </button>
                </div>
              ...
              <Button
                onClick={handleBuy}
                disabled={isLoading || !amount || !!error || isRestricted}
                className={`w-full mt-6 h-12 rounded-xl text-sm font-bold shadow-lg shadow-brand-500/20 transition-all active:scale-[0.98] ${
                  isRestricted
                    ? "bg-gray-300 text-gray-500 dark:bg-white/10 dark:text-white/40 cursor-not-allowed shadow-none"
            ...
            <div className="sticky top-24 bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-[1.5rem] p-6 shadow-xl overflow-hidden">
              ...
              <div className="space-y-4 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-mono text-gray-900 dark:text-white font-medium">
                    {quantity || 0} {currentCompany.ticker}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500 dark:text-gray-400">
                ...
                <div className="flex justify-between items-end">
                  ...
                  <span
                    className={`text-2xl font-mono font-bold ${isRestricted ? "text-gray-400" : "text-brand-600 dark:text-brand-400"}`}
                  >
                    $
              ...
              <div className="mt-6 p-3 rounded-xl bg-gray-50 dark:bg-black/20 text-xs text-gray-500 dark:text-gray-400 leading-relaxed text-center">
                Prices are indicative. Actual execution price may vary slightly
